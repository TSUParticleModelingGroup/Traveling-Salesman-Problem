...
//Move wall by a set number because the wall will be not be a fixed distance apart
//nvcc TravelingSalesManComparison34.cu -o TSPCompare34 -lglut -lGL -lm
#include <GL/glut.h>
#include <math.h>
#include <stdio.h>
#include <stdlib.h>
...
#define WALL_STRENGTH 2000.0
#define FORCE_CUT_OFF (WALL_STRENGTH*0.6)

// Ideal number of nodes in radius 1 circle.
#define IDEAL_NUMBER_OF_NODES 16.0

#define P 13
#define Q 1

#define SLOPE_REPULSION -25.0
#define	SLOPE_ATRACTION -0.2
...
#define PRINT_GEOMETRIC_CENTER 1
#define PRINT_NODE_ADJUSTMENT_FACTOR 1
...
float4 findCenterOfMass(float4 *pos, int n);
float findOuterRadius(float4 *pos, int n);
void drawPoints(float4 pos, int n);
void drawNbodyExtrusion(float4 *pos, float4 center, float outerRadius, float innerRadius, int n);
void getPathNbody(float4 *pos, float4 centerOfMass, int* path, int n);
...
float adjustingNodes(float4 *pos, int numberOfNodes)
{
	...
	idealRadius = numberOfNodes/IDEAL_NUMBER_OF_NODES;
	
	nodeAdjustmentFactor = idealRadius/distanceToOutermostNode;
...
{
	int i;
	float normalizingFactor = ((float)n)/IDEAL_NUMBER_OF_NODES;
...
__global__ void accelerationsNbody(float4 *nodePos, float4 *pos, float4 *vel, float4 *acc, float *mass, float4 centerOfMass, float innerRadius, float outerRadius, int n)
{
	int j,ii;
	...
	if(id < n)
	{
		...
		dx = posMe.x - centerOfMass.x;
		dy = posMe.y - centerOfMass.y; 
		radius = sqrtf(dx*dx + dy*dy);
	
		if(radius < innerRadius) // Inside inner wall
		{
			if(radius > FLOAT_ROUND_OFF)
			{
				forceMag = WALL_STRENGTH*(innerRadius - radius);
				forceSum.x += forceMag*dx/radius;
				forceSum.y += forceMag*dy/radius;
			}
			// If a node starts very close to or on (0,0). There is no vector to push it down.
			// Hence, it is just pushed in the <1,1> direction. 
			// The wall strenght is redues because there 
			// is no distance inside the wall to multiple by.
			else
			{
				dx = 1.0;
				dy = 1.0;	
				...
				// Creating a small wall strength
				forceMag = WALL_STRENGTH*innerRadius;
...
float NbodyExtrusionTSP(float4 *nodePos, float4 *pos, float4 *vel, float4 *acc, float* mass, int* path, int n)
{
	...
	// Setting up the initial run conditions.
	time = 0.0;
	draw_count = 0;
	centerOfMass = findCenterOfMass(pos, n);
	innerRadius = 0.0;
	outerRadius = findOuterRadius(pos, n);
	printf("\n\n  outerRadius = %f perimiter = %f", outerRadius, 2.0*PI*outerRadius);
	float temp = sqrtf((pos[0].x - pos[1].x)*(pos[0].x - pos[1].x) + (pos[0].y - pos[1].y)*(pos[0].y - pos[1].y));
	printf("\n\n  edge length = %f total edge lenght = %f\n", temp, temp*n);
	drawNbodyExtrusion(pos, centerOfMass, outerRadius, innerRadius, n);
	...
	while(innerRadius + 0.01 < outerRadius)
	{
		while(time < TIME_BETWEEN_WALL_MOVES)
		{		
			accelerationsNbody<<<grid, block>>>(nodePosGPU, posGPU, velGPU, accGPU, massGPU, centerOfMass, innerRadius, outerRadius, n);
		...
		time = 0.0;
		innerRadius = innerRadius + dr;
		outerRadius = outerRadius;
	}
	
	getPathNbody(pos, centerOfMass, path, n);
...
void control()
{
	...
	{	
		...
		if(PRINT_NODE_ADJUSTMENT_FACTOR == 1)
		{
		...
		printf("\n  Deterining the exact cost is done.");
