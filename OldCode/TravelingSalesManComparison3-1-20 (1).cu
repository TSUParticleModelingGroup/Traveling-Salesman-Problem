...
__global__ void moveNbody(float4 *pos, float4 *vel, float4 *acc, float viscosity, float dt, int n)
{
    int id = threadIdx.x + blockDim.x*blockIdx.x;
    if(id < n)
    {
	    vel[id].x += (acc[id].x -viscosity*vel[id].x)*dt;
		vel[id].y += (acc[id].y -viscosity*vel[id].y)*dt;
	...
	while(annealingCount <= NUMBER_OF_ANEALINGS)
	{
		...
		//while(innerRadius + stopSeperation < dr)
		{
			...
			{		
				...
				moveNbody<<<grid, block>>>(posGPU, velGPU, accGPU, viscosity, dt, n);
