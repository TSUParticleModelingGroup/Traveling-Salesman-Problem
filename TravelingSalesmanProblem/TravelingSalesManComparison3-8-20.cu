...
//nvcc TravelingSalesManComparison3-8-20.cu -o temp -lglut -lGL -lm -use_fast_math
#include <GL/glut.h>
#include <math.h>
#include <stdio.h>
#include <stdlib.h>
...
#define FORCE_FUNTION_TYPE 2 // 0 for LJ-HPQ. 1 for LJ-MPQ. 2 Linear. 3 for billards.
...
void drawNbodyExtrusion(float4 *pos, float innerRadius, float outerRadius, int innerWallDirection, int outerWallDirection, int n, float4 bubbles);
...
__global__ void accelerationsNbody(float4 *node, float4 *pos, float4 *vel, float4 *acc, float *mass, float innerRadius, float outerRadius, int n, float4 bubbles, float viscosity)
{
	int j,ii;
    float3 forceSum;
    float4 nodeMe, posMe;
	...
	if(id < n)
	{
		// Forces between the node and the walls
		dx = posMe.x;
		dy = posMe.y; 
		...
		// Forces between the node and the bubbles
		dx = posMe.x - bubbles.x;
		dy = posMe.y - bubbles.y; 
		radius = sqrtf(dx*dx + dy*dy);
		if(radius < innerRadius) // Inside inner wall
		{
			forceMag = WALL_STRENGTH*(bubbles.w - radius);
			forceSum.x += forceMag*dx/radius;
			forceSum.y += forceMag*dy/radius;
		}
		
		// Adding on damping force.
		forceSum.x += -viscosity*vel[id].x;
		forceSum.y += -viscosity*vel[id].y;
...
double NbodyExtrusionTSP(float4 *node, float4 *pos, float4 *vel, float4 *acc, float* mass, int* path, int n)
{
	...
	while(annealingCount <= NUMBER_OF_ANEALINGS)
	{
		...
		stopSeperation = findMinimumSeperation(pos, n)/2.0;
		
		// Setting up bubbles
		bubbles.x = findDistanceToOuterMostElement(pos, n)/3.0f;
		bubbles.y = findDistanceToOuterMostElement(pos, n)/3.0f;
		bubbles.z = 1.0;
		bubbles.w = 0.0;
		
		innerRadius = 0.0;
		outerRadius = findDistanceToOuterMostElement(pos, n);
		drawNbodyExtrusion(pos, innerRadius, outerRadius, 0, 0, n, bubbles);
		...
		//while(innerRadius + stopSeperation < dr)
		{
			...
			bubbles.w += bubblesDr*innerWallDirection;
			
			time = 0.0;
			while(time < TIME_BETWEEN_WALL_MOVES)
			{		
				accelerationsNbody<<<grid, block>>>(nodeGPU, posGPU, velGPU, accGPU, massGPU, innerRadius, outerRadius, n, bubbles, viscosity);
				...
				if(draw_count == STEPS_BETWEEN_VIEWING)
				{
					...
					drawNbodyExtrusion(pos, innerRadius, outerRadius, innerWallDirection, outerWallDirection, n, bubbles);
					draw_count = 0;
				}
				draw_count++;
				time += dt;
		...
		printf("\n  anneal %d pathcost = %f\n", annealingCount, pathCost);
		
		if(annealingCount < NUMBER_OF_ANEALINGS)
		{
			viscosity = 1.0;
			innerRadius = 0.0;
			bubbles.w = innerRadius;
...
void drawNbodyExtrusion(float4 *pos, float innerRadius, float outerRadius, int innerWallDirection, int outerWallDirection, int n, float4 bubbles)
{
	...
	bubbles.x /= normalizingFactor;
	bubbles.y /= normalizingFactor;
	bubbles.z /= normalizingFactor;
	bubbles.w /= normalizingFactor;
	...
	glBegin(GL_LINE_LOOP);
		for(i = 0; i <= lineAmount;i++) 
		{ 
			glVertex2f(x_world_to_x_screen(bubbles.x + bubbles.w*cos(i*twicePi/lineAmount)), 
			           y_world_to_y_screen(bubbles.y + bubbles.w*sin(i*twicePi/lineAmount)));
		}
	glEnd();
